...
![pdf_search ツール呼び出し](docs/demo-1.png)
*① MCP クライアントが `pdf_search` を呼び出し — クエリと top_k パラメータ。*

![検索結果に基づく応答](docs/demo-2.png)
*② 取得したチャンクで回答 — 出典・スコア・マッチ経路(hybrid)を表示。*

以下はテキストで見た検索例です。(スコア・チャンク番号は例示であり、環境により異なります。)
